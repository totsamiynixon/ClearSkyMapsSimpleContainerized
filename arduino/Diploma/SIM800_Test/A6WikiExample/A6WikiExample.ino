...
//  Следующие 2 строки нужны для работы по программной шине UATR (выводы 7 и 8):              //
    ...
    SoftwareSerial softSerial(6,7);                                                           // Создаём объект softSerial указывая выводы RX и TX платы Arduino.
...
//  gsm.TXTreadCoding(GSM_TXT_CP866);                                                         // Если символы Кириллицы входящих  SMS сообщений некорректно отображаются в мониторе,  выберите одну из кодировок: GSM_TXT_CP866 / GSM_TXT_UTF8 / GSM_TXT_WIN1251.
    ...
    gsm.SMSsend(F("Это обычное SMS сообщение"), "375259132721");                               // Отправка обычного (короткого) SMS сообщения на номер +7(012)345-67-89.
//                                                                                              //
//    Serial.println(F("Sending long SMS ..."));                                                // Отправка составного (состоящего из нескольких SMS) сообщения на номер +7(012)345-67-89.
//    gsm.SMSsend(F("Данное SMS сообщение является составным, так как содержит более 70"), "375259132721", 0x1234, 3, 1); // синтаксис: gsm.SMSsend("текст", "адрес получателя", идентификатор составного SMS сообщения, количество SMS в составном сообщении, номер SMS в составном сообщении).
//    gsm.SMSsend(F(" символов. По этому это сообщение будет отправлено как 3 SMS сообщ"), "375259132721", 0x1234, 3, 2); // синтаксис: gsm.SMSsend("текст", "адрес получателя", идентификатор составного SMS сообщения, количество SMS в составном сообщении, номер SMS в составном сообщении).
//    gsm.SMSsend(F("ения.\nНо получатель увидит его как один большой целый текст."     ), "375259132721", 0x1234, 3, 3); // синтаксис: gsm.SMSsend("текст", "адрес получателя", идентификатор составного SMS сообщения, количество SMS в составном сообщении, номер SMS в составном сообщении).
//                                                                                              //
//    Serial.print(F("Sending normal SMS with verification ... "));                             //
//    if(gsm.SMSsend(F("Это обычное SMS сообщение"), "375259132721"))                            // Отправка обычного (короткого) SMS сообщения на номер +7(012)345-67-89.
...
//                                                                                              //
    ...
    gsm.SMSsend(F("Это SMS сообщение класса 0"), "375259132721");                              // Отправка SMS сообщения на номер +7(012)345-67-89, которому будет присвоен 0 класс.
